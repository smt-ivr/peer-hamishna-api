...
export async function handleYemotManager(request, env) {
    ...
    // שלב 2: קבלת קוד המבחן (לדוגמה: 1*1 למבחן 1א)
    if (!examInput) {
        return new Response(`read=t-שלום ${student.first_name} ${student.last_name}. הקישו את מספר המבחן, מקש כוכבית, ואז 1 לאות אלף או 2 לאות בית, ולסיום סולמית=exam_input,,,,,NO,,,,,,,,,no`, { 
            headers: { 'Content-Type': 'text/plain; charset=utf-8' } 
        });
    }
    ...
    if (!passInput) {
        ...
        // בקשת הציון עצמו
        return new Response(`read=t-מבחן ${examCode}. לעדכון שעבר הקישו 1. לא עבר הקישו 0.=pass_input,,1,,,NO,,,,01,,,,,no`, { 
            headers: { 'Content-Type': 'text/plain; charset=utf-8' } 
        });
    }
    ...
    try {
        const passedValue = passInput === '1' ? 1 : 0;
